# Исправление ошибок подключения к API

## Проблема
- `ERR_CONNECTION_TIMED_OUT` при запросах к `api.bigo.1tlt.ru`
- 404 ошибки

## ✅ Что исправлено в коде:

1. **Все импорты axios заменены на кастомный клиент** с таймаутом 30 секунд
2. **Улучшена обработка ошибок** - теперь логируются детали проблем подключения
3. **Все файлы используют единый axios клиент** из `frontend/web/lib/axios.js`

## 🔍 Диагностика проблемы:

### 1. Проверка доступности API

Выполните на сервере:

```bash
# Проверка локально
curl http://localhost:5000/api/health

# Проверка через внешний домен
curl https://api.bigo.1tlt.ru/api/health
```

**Ожидаемый результат:** `{"status":"ok","timestamp":"..."}`

### 2. Проверка статуса бэкенда

```bash
pm2 status
pm2 logs bigo-backend --lines 30
```

### 3. Проверка переменных окружения на Vercel

В **Vercel Dashboard → Settings → Environment Variables** должны быть:

- `NEXT_PUBLIC_API_URL` = `https://api.bigo.1tlt.ru`
- `NEXT_PUBLIC_SOCKET_URL` = `https://api.bigo.1tlt.ru`

**Важно:** После изменения переменных окружения нужно **пересобрать проект** на Vercel.

### 4. Проверка CORS на сервере

В `.env` на сервере должно быть:

```bash
CORS_ORIGIN=https://bigo-mvp.vercel.app,https://bigo-mvp.vercel.app/*
```

Или временно для теста:

```bash
CORS_ORIGIN=*
```

После изменения:

```bash
pm2 restart bigo-backend --update-env
```

### 5. Проверка Nginx (если используется)

Убедитесь, что:
- Проксирование настроено на `http://localhost:5000`
- CORS заголовки правильно настроены
- Нет блокировки запросов

### 6. Проверка Firewall

Убедитесь, что порт 5000 (или 443 для HTTPS) открыт:

```bash
# Проверка открытых портов
netstat -tlnp | grep 5000
# или
ss -tlnp | grep 5000
```

## 🛠 Решение проблем:

### Если API недоступен:

1. **Проверьте, запущен ли бэкенд:**
   ```bash
   pm2 restart bigo-backend --update-env
   ```

2. **Проверьте логи на ошибки:**
   ```bash
   pm2 logs bigo-backend --err --lines 50
   ```

3. **Проверьте подключение к MongoDB:**
   - Убедитесь, что `MONGODB_URI` правильный в `.env`
   - Проверьте доступность MongoDB

### Если ERR_CONNECTION_TIMED_OUT:

1. **Проверьте DNS:**
   ```bash
   nslookup api.bigo.1tlt.ru
   ping api.bigo.1tlt.ru
   ```

2. **Проверьте SSL сертификат:**
   ```bash
   curl -v https://api.bigo.1tlt.ru/api/health
   ```

3. **Проверьте настройки Nginx/Reverse Proxy**

## 📝 После исправления:

1. Обновите переменные окружения на Vercel
2. Пересоберите проект на Vercel
3. Проверьте работу сайта



